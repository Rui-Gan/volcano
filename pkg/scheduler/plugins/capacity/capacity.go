...
func (cp *capacityPlugin) createOrUpdateRootQueue(vcClient vcclient.Interface) error {
	...
	oldTotalResource := api.NewResource(queue.Spec.Deserved)
	oldGuaranteedResource := api.NewResource(queue.Spec.Guarantee.Resource)
	if cp.totalResource.Equal(oldTotalResource, api.Zero) && oldTotalResource.Equal(cp.totalResource, api.Zero) &&
		cp.totalGuarantee.Equal(oldGuaranteedResource, api.Zero) && oldGuaranteedResource.Equal(cp.totalGuarantee, api.Zero) {
		return nil
	}
